...
class stream
{
	...
	public:
		...
		stream(std::FILE* fh, no_ownership_t)
			: fh(fh)
			, m_unowned(true)
		{ }
		...
		bool open(const char* filename, int flags)
		{
			...
			fh = std::fopen(filename, impl::flags_to_modestr(flags).data());
		...
		void reset(std::FILE* new_fh = nullptr)
		{
			fh = new_fh;
		}
...
namespace std
{
	template <> struct hash<::cio::stream>
	{
		...
		size_t operator()(const ::cio::stream& io) const
		{
	...
	{
		typedef ::cio::stream::mode_t argument_type;
		typedef size_t result_type;
		
		size_t operator()(const ::cio::stream::mode_t& mode) const
		{
			using UT = underlying_type<::cio::stream::mode_t>::type;
			return hash<UT>()(mode);
		}
	};
}
