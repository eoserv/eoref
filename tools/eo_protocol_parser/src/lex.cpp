...
static bool ctype_symbol(char c) { return ctype(":;{}.,[]", c); }
static bool ctype_whitespace(char c) { return std::isspace(c); }
static bool ctype_alpha(char c) { return std::isalpha(c); }
static bool ctype_digit(char c) { return std::isdigit(c); }
static bool ctype_alnum(char c) { return ctype_alpha(c) || ctype_digit(c); }
static bool ctype_ident_start(char c) { return ctype_alpha(c) || c == '_' || c == '$'; }
static bool ctype_ident(char c) { return c != '$' && (ctype_ident_start(c) || ctype_digit(c)); }

static bool check_op(char a, char b)
{
	switch (a)
	{
...
Token Lexer::ReadCharacter()
{
	...
	return Token(Token::Character, s);
}
...
Token Lexer::ReadToken()
{
	...
	if (this->PeekChar(c))
	{
		...
		else if (ctype_op(c)) // Assumes '/' is a valid operator
		{
			...
			if (int(token) == UOP2('/', '\0') && this->PeekChar(cc) && cc == '/') // Line-comments
			{
				while (c != '\n')
					this->GetChar(c);
